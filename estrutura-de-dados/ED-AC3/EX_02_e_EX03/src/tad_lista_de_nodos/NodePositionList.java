...
// Exercicio 11 AC2 - Retorna a lista invertida.
...
//	Exercicio 12 AC2 - Move o elemento na posição p para a primeira posição, mantendo a ordem relativa dos demais elementos inalterada.
	public void makeFirst(Position<E> p) {
		DNode<E> temp =  (DNode<E>) p;
		DNode<E> prev = temp.getPrev();
		DNode<E> next = temp.getNext();
		DNode<E> first = header.getNext();
		prev.setNext(next);
		next.setPrev(prev);
		header.setNext(temp);
		addAfter(header.getNext(), first.element());
	}
}
